...
if ticker:
    try:
        ...
        # ✅ Rensa och kontrollera att alla viktiga kolumner finns
        expected_cols = ["Open", "High", "Low", "Close", "Volume"]
        for col in expected_cols:
            if col not in data.columns:
                raise ValueError(f"Datan saknar kolumnen: {col}")

        # ✅ Säkerställ att alla värden är numeriska och inga NaN finns
        data = data[expected_cols].apply(pd.to_numeric, errors='coerce')
        ...
        df = data.copy()
        df.index.name = 'Date'
        df = df[-100:]  # Senaste 100 datapunkter
